...
namespace codeEditor.NavigatePanel
{
    public class FolderNode : NavigatePanelNode
    {
        ...
        public override void DrawNode(Graphics graphics, int x, int y, Font font, Color color, Color backgroundColor, Color selectedColor, int lineHeight, bool selected)
        {
            if (Exanded)
            {
                graphics.DrawImage(openFolder.GetImage(lineHeight, ajkControls.Icon.ColorStyle.Blue), new Point(x, y));
            }
            else
            {
                graphics.DrawImage(folder.GetImage(lineHeight, ajkControls.Icon.ColorStyle.Blue), new Point(x, y));
            }
